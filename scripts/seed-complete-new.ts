...
async function seedComplete() {
  ...
  try {
    ...
    // Vérifier d'abord si exec_sql existe, si non on guide l'utilisateur
    const { error: testExecSqlError } = await supabase.rpc("exec_sql", { sql: "select 1" });

    if (testExecSqlError && testExecSqlError.message.includes("function exec_sql")) {
      console.log("   ⚠️ Base de données pas encore configurée");
      console.log("   👉 Exécutez ce script dans Supabase SQL Editor :");
      console.log("      scripts/setup-database.sql");
      console.log("   ⏸️  Puis relancez ce seed");
      return;
    }

    // Vérifier si la colonne school_year_id existe dans payments
    const { error: testError } = await supabase.from("payments").select("school_year_id").limit(1);
    ...
    if (testError) {
      ...
      if (message.includes('column "school_year_id" does not exist')) {
        console.log("   ⚠️ Colonne school_year_id absente, tentative d'ajout via exec_sql...");
        ...
        if (alterError) {
          const alterMessage = alterError.message ?? "";

          console.log("   ❌ Ajout automatique impossible:", alterMessage);
          if (alterMessage.includes("function exec_sql")) {
            console.log("   ℹ️  La fonction exec_sql n'est pas disponible.");
            console.log("   👉 Créez-la dans Supabase SQL Editor :");
            console.log(
          ...
          console.log("   👉 Puis exécutez le script SQL suivant pour ajouter la colonne :");
          console.log("      scripts/add-school-year-to-payments.sql");
          console.log(
            "   💡 Après modification, exécutez : SELECT pg_notify('pgrst', 'reload schema');"
          );

          throw new Error(
            "La table payments ne possède pas la colonne school_year_id. Ajoutez-la avant de relancer le seed."
          );
        }
        ...
        await supabase
          .rpc("exec_sql", { sql: "SELECT pg_notify('pgrst', 'reload schema');" })
          .catch(() => {
            console.log("   ℹ️ Notification pgrst non envoyée (optionnelle).");
          });
      } else {
        console.error("   ❌ Erreur inattendue en vérifiant payments.school_year_id:", message);
        throw testError;
      }
    } else {
      ...
      for (const firstName of names) {
        const studentData = {
          ...
          birth_date: faker.date
            .between({ from: "2008-01-01", to: "2018-12-31" })
            .toISOString()
            .split("T")[0],
        ...
        if (!studentError && student) {
          ...
          if (currentYear) {
            enrollmentPlans.push({
              year: currentYear,
              courses: faker.helpers.arrayElements(
                createdCourses,
